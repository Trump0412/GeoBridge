...
from .msgf_utils import FrameLayout, compute_stage_ranges
...
class Qwen2_5_VLVisionSdpaAttention(nn.Module):
    ...
    def forward(
        ...
        # The vision tower receives concatenated image/video windows. Building a
        # dense global [seq, seq] SDPA mask is prohibitively expensive for
        # multi-frame training samples, so compute the same block-diagonal
        # attention by slicing on the cumulative sequence lengths.
...
def _normalize_mrope_position_ids(position_ids: Optional[torch.LongTensor], batch_size: int):
    if position_ids is None or position_ids.dim() != 3:
        return position_ids

    # Some generation stacks prepend text position ids to Qwen-VL's three
    # vision axes. The Qwen2.5-VL rotary block consumes only
    # temporal/height/width as [3, batch, seq].
    if position_ids.shape[0] == 4:
        return position_ids[1:]
    if position_ids.shape[1] == 4 and position_ids.shape[0] == batch_size:
        return position_ids[:, 1:, :].permute(1, 0, 2).contiguous()
    if position_ids.shape[0] == 3:
        return position_ids
    if position_ids.shape[1] == 3 and position_ids.shape[0] == batch_size:
...
QWEN2_5_VL_GEOMETRY_ATTENTION_CLASSES = {
        ...
        if self.cross_flag:
            ...
            if self.cross_flag:
                self.cross_attn = QWEN2_5_VL_GEOMETRY_ATTENTION_CLASSES[self.geo_inject_version](
                    ...
                    use_spatial_bias=getattr(config, 'geo_spatial_bias', False), 
                    geo_learn_bias=getattr(config, 'geo_learn_bias', False),
                    
                    use_importance_gate=getattr(config, 'geo_importance_gate', False),
    ...
    def forward(
        ...
        **kwargs,
        ...
        if self.cross_flag and geo_embed_mask is not None:
            ...
            question_types = bank_question_types if bank_question_types is not None else kwargs.get("bank_question_types")
            ...
            for batch_idx in range(batch_size):
                ...
                sample_question_types = question_types
                if isinstance(question_types, (list, tuple)) and len(question_types) == batch_size:
                    sample_question_types = [question_types[batch_idx]]
...
class Qwen2_5_VLModel(Qwen2_5_VLPreTrainedModel):
    ...
    def forward(
        ...
        if position_ids is None:
            ...
            position_ids = _normalize_mrope_position_ids(position_ids, inputs_embeds.shape[0])
        ...
        for decoder_layer in self.layers:
            ...
            if self.gradient_checkpointing and self.training:
                ...
                layer_outputs = decoder_layer(
                    ...
                    cache_position=cache_position,
                    position_embeddings=position_embeddings,
                    geo_embeds=geo_embeds,
                    geo_embed_mask=geo_embed_mask,
                    image_grid_thw=image_grid_thw,
                    bank_question_types=bank_question_types,
                )
...
class Qwen2_5_VLForConditionalGenerationWithVGGT(Qwen2_5_VLPreTrainedModel, GenerationMixin):
    ...
    def _init_geometry_encoder(self, config):
        ...
        self.geometry_merger = GeometryFeatureMerger(
            ...
            depart_smi_token=getattr(config, "depart_smi_token", False),
            smi_downsample_rate=getattr(self.config, 'smi_downsample_rate', 2),
        )
    ...
    def _process_geometry_features(self, image_embeds, geometry_encoder_inputs, inputs_embeds, image_grid_thw=None):
        ...
        if getattr(self.config, "geo_inject_version", "") in {"zenview_vggt_bank", "zenview_continuity_bank_v2", "geobridge_hgb"}:
            ...
            return image_embeds, geometry_banks, split_image_grid_thw
        ...
        if geo_embeds is not None:
            ...
            if getattr(self.config, 'depart_smi_token', False) and n_image > getattr(self.config, 'smi_image_num', 8): 
                h_patch = height // self.geometry_encoder.patch_size // 2
                w_patch = width // self.geometry_encoder.patch_size // 2
                smi_downsample_rate = getattr(self.config, 'smi_downsample_rate', 2)
                # --- grid alignment patch (keep frames, support varying visual grids) ---
                dim = geo_embeds.shape[-1]
                tokens = image_embeds.numel() // dim
                tokens_per_frame = tokens // n_image

                import math
                target_ratio = (w_patch / h_patch) if h_patch > 0 else 1.0
                best = None
                for h in range(1, int(math.sqrt(tokens_per_frame)) + 1):
                    if tokens_per_frame % h != 0:
                        continue
                    w = tokens_per_frame // h
                    r = w / h  # enforce w>=h by construction
                ...
                # if visual grid != geo grid, resize geo_embeds to visual grid
                if (h_img != h_patch) or (w_img != w_patch):
                    geo = geo_embeds.permute(0, 3, 1, 2)  # (n, C, H, W)
                    geo = F.interpolate(geo, size=(h_img, w_img), mode="bilinear", align_corners=False)
                    geo_embeds = geo.permute(0, 2, 3, 1)  # (n, H, W, C)

                image_embeds = image_embeds.reshape(n_image, h_img, w_img, dim)
                # --- end patch ---

                image_embeds = image_embeds.permute(0, 3, 1, 2)
                image_embeds = torch.nn.functional.interpolate(
                    image_embeds, 
                    size=(h_patch // smi_downsample_rate, w_patch // smi_downsample_rate), 
                    mode="bilinear", 
                    align_corners=False
                )
                image_embeds = image_embeds.permute(0, 2, 3, 1)
            elif hasattr(self.config, "feature_fusion_method") and self.config.feature_fusion_method == "zero":
                pass
            else:
        ...
        return image_embeds, features_3d, image_grid_thw

    @staticmethod
    def _reload_local_safetensors_checkpoint(model, pretrained_model_name_or_path):
        """Reload local shards explicitly; avoids silent no-op loads in this custom HF class."""
        import json
        import os
        ...
        if os.path.isfile(index_path):
            ...
            for filename in index.get("weight_map", {}).values():
                if filename not in seen:
                    shard_files.append(os.path.join(model_dir, filename))
                    seen.add(filename)
        else:
            shard_files = [
        ...
        shard_files = [path for path in shard_files if os.path.isfile(path)]
        if not shard_files:
            return

        def set_module_tensor(module, tensor_name, tensor, is_parameter):
            target_module = module
            parts = tensor_name.split(".")
            for part in parts[:-1]:
                target_module = getattr(target_module, part)
            if is_parameter:
                target_module._parameters[parts[-1]] = nn.Parameter(tensor)
            else:
                target_module._buffers[parts[-1]] = tensor

        parameters = dict(model.named_parameters())
        buffers = dict(model.named_buffers())
        state_shapes = {
            **{name: tuple(tensor.shape) for name, tensor in parameters.items()},
            **{name: tuple(tensor.shape) for name, tensor in buffers.items()},
        }
        loaded = 0
        skipped_missing = 0
        skipped_shape = 0
        ...
        with torch.no_grad():
            for shard_path in shard_files:
                ...
                for name, tensor in shard_state.items():
                    target_shape = state_shapes.get(name)
                    ...
                    if target_shape != tuple(tensor.shape):
                        ...
                        if len(shape_examples) < 8:
                            shape_examples.append((name, target_shape, tuple(tensor.shape)))
                        continue
                    if name in parameters:
                        target = parameters[name]
                        tensor = tensor.to(dtype=target.dtype)
                        if target.is_meta:
                            set_module_tensor(model, name, tensor, is_parameter=True)
                            parameters[name] = dict(model.named_parameters())[name]
                        else:
                        ...
                        tensor = tensor.to(dtype=target.dtype)
                        if target.is_meta:
                            set_module_tensor(model, name, tensor, is_parameter=False)
                            buffers[name] = dict(model.named_buffers())[name]
                        else:
                            target.data.copy_(tensor.to(device=target.device))
                        loaded += 1

        print(
            "[GeoThinker Load] safetensors reload "
            f"loaded={loaded}, skipped_missing={skipped_missing}, "
            f"skipped_shape={skipped_shape}, shards={len(shard_files)}"
        )
        if shape_examples:
            print("[GeoThinker Load] safetensors reload shape examples:", shape_examples)
    ...
    def from_pretrained(cls, pretrained_model_name_or_path, *model_args, **kwargs):
        ...
        print(f"[GeoThinker Load] loading base model from {pretrained_model_name_or_path}")
        ...
        print("[GeoThinker Load] base model loaded")
        cls._reload_local_safetensors_checkpoint(model, pretrained_model_name_or_path)
        ...
        if geometry_encoder_path:
            print(f"[GeoThinker Load] loading geometry encoder from {geometry_encoder_path}")
            model.geometry_encoder.load_model(geometry_encoder_path)
            print("[GeoThinker Load] geometry encoder loaded")
    ...
    def get_rope_index(
        ...
        if getattr(self.config, 'depart_smi_token', False) and num_images > getattr(self.config, 'smi_image_num', 8):
            spatial_merge_size *= getattr(self.config, 'smi_downsample_rate', 2)
    ...
    def forward(
        ...
        **kwargs,
        ...
        geo_embeds, geo_embed_mask = None, None
        if geometry_encoder_inputs is not None:
            geometry_encoder_inputs = self._normalize_geometry_encoder_inputs(geometry_encoder_inputs)
        decoder_image_grid_thw = image_grid_thw if image_grid_thw is not None else video_grid_thw
        if inputs_embeds is None:
            inputs_embeds = self.model.embed_tokens(input_ids)
            input_ids_for_masks = input_ids.to(inputs_embeds.device)
            labels_for_masks = labels.to(inputs_embeds.device) if labels is not None else None
            if pixel_values is not None:
                pixel_values = pixel_values.type(self.visual.dtype)
                image_embeds = self.visual(pixel_values, grid_thw=image_grid_thw)


                mask = input_ids_for_masks == self.config.image_token_id
                mask_unsqueezed = mask.unsqueeze(-1)
                mask_expanded = mask_unsqueezed.expand_as(inputs_embeds)
                image_mask = mask_expanded.to(inputs_embeds.device)
                geo_embed_mask = image_mask.clone()

                vision_mask = (
                    mask
                    | (input_ids_for_masks == self.config.vision_start_token_id)
                    | (input_ids_for_masks == self.config.vision_end_token_id)
                )

                if labels is not None:
                    text_embeds = inputs_embeds[(vision_mask == False) & (labels_for_masks == -100)].view(1,-1,self.config.hidden_size)
                else: text_embeds = inputs_embeds[vision_mask == False].view(1,-1,self.config.hidden_size)
                ...
                if getattr(self.config, 'use_geometry_encoder', False) and geometry_encoder_inputs is not None:
                    image_embeds, geo_embeds, decoder_image_grid_thw = self._process_geometry_features(
                        image_embeds,
                        geometry_encoder_inputs,
                        text_embeds,
                        image_grid_thw=image_grid_thw,
                    )
                    if geo_embeds is not None and hasattr(geo_embeds, "bank"):
                        pass
                    elif isinstance(geo_embeds, (list, tuple)):
                        pass
                    elif geo_embeds is not None:
            ...
            if pixel_values_videos is not None:
                ...
                mask = input_ids_for_masks == self.config.video_token_id
                n_video_tokens = mask.sum().item()
        ...
        if position_ids is None and (attention_mask is None or attention_mask.ndim == 2):
            ...
            if (
                ...
                first_geometry_input = (
                    geometry_encoder_inputs[0]
                    if isinstance(geometry_encoder_inputs, (list, tuple)) and len(geometry_encoder_inputs) > 0
                    else geometry_encoder_inputs
                )
                position_ids, rope_deltas = self.get_rope_index(
                    ...
                    num_images=max(
                        1,
                        self._geometry_input_num_images(first_geometry_input),
                    ),
                )
        ...
        outputs = self.model(
            ...
            geo_embeds=geo_embeds,
            geo_embed_mask=geo_embed_mask,
            image_grid_thw=decoder_image_grid_thw,
            bank_question_types=kwargs.get("bank_question_types"),
        )
        ...
        if labels is not None:
            ...
            if getattr(self.config, 'use_qwenvl_loss', False):
                loss = self.loss_function(logits=logits, labels=labels, vocab_size=self.config.vocab_size, **kwargs)
            else:
                loss_fct = CrossEntropyLoss()
                shift_labels = labels[..., 1:].to(logits.device)
                valid_label_mask = shift_labels != -100
                if bool(valid_label_mask.any()):
                    shift_logits = logits[..., :-1, :][valid_label_mask].float()
                    loss = loss_fct(shift_logits, shift_labels[valid_label_mask])
                else:
                    loss = logits.sum() * 0.0
...
__all__ = ["Qwen2_5_VLForConditionalGenerationWithVGGT", "Qwen2_5_VLModel", "Qwen2_5_VLPreTrainedModel"]
