...
QWEN3_VL_GEOMETRY_ATTENTION_CLASSES = {
...
class Qwen3VLTextDecoderLayer(nn.Module):
    def __init__(self, config: Qwen3VLTextConfig, layer_idx: int):
        ...
        if self.cross_flag:
            ...
            if self.cross_flag:
                self.cross_attn = QWEN3_VL_GEOMETRY_ATTENTION_CLASSES[self.geo_inject_version](
...
class Qwen3VLModel(Qwen3VLPreTrainedModel):
    ...
    def _process_geometry_features(self, image_embeds, geometry_encoder_inputs, inputs_embeds, image_grid_thw, deepstack_image_embeds):
        ...
        if geo_embeds is not None:
            ...
            merge_size = self.config.vision_config.spatial_merge_size
            if getattr(self.config, 'depart_smi_token', False) and geo_embeds.shape[0] > getattr(self.config, 'smi_image_num', 8):
                merge_size *= getattr(self.config, 'smi_downsample_rate', 2)

            if self.config.vision_config.patch_size != self.geometry_encoder.patch_size:
                geo_embeds = geo_embeds.permute(0, 3, 1, 2)
                geo_embeds = F.interpolate(geo_embeds, size=(
                    image_grid_thw[0][1]//merge_size, 
                    image_grid_thw[0][2]//merge_size,
                ), mode='bilinear', align_corners=False)
                geo_embeds = geo_embeds.permute(0, 2, 3, 1)

            n_image, h_patch, w_patch, dim = geo_embeds.shape
            if getattr(self.config, 'depart_smi_token', False) and n_image > getattr(self.config, 'smi_image_num', 8):
                h_patch_ori = image_grid_thw[0][1] // self.config.vision_config.spatial_merge_size
                w_patch_ori = image_grid_thw[0][2] // self.config.vision_config.spatial_merge_size

                image_embeds = image_embeds.view(n_image, h_patch_ori, w_patch_ori, dim)
                ...
                for image_index in range(len(deepstack_image_embeds)):
                    this_image_embeds = deepstack_image_embeds[image_index].view(n_image, h_patch_ori, w_patch_ori, dim)
                    this_image_embeds = this_image_embeds.permute(0, 3, 1, 2)
                    this_image_embeds = torch.nn.functional.interpolate(
                        this_image_embeds, 
                        size=(h_patch, w_patch), 
                        mode="bilinear", 
                        align_corners=False
                    )
                    this_image_embeds = this_image_embeds.permute(0, 2, 3, 1)
                    deepstack_image_embeds[image_index] = this_image_embeds.view(-1, dim)

            elif self.config.feature_fusion_method != "zero":
        ...
        return image_embeds, features_3d, deepstack_image_embeds
    ...
    def get_placeholder_mask(
        ...
        depart_smi_token = hasattr(self.config, "depart_smi_token") and self.config.depart_smi_token

        n_image_tokens = special_image_mask.sum()
        special_image_mask = special_image_mask.unsqueeze(-1).expand_as(inputs_embeds).to(inputs_embeds.device)
        
        if depart_smi_token and num_images > getattr(self.config, 'smi_image_num', 8):
            pass
        elif image_features is not None and inputs_embeds[special_image_mask].numel() != image_features.numel():
            raise ValueError(
        ...
        if depart_smi_token and num_images > getattr(self.config, 'smi_image_num', 8):
            pass
        elif video_features is not None and inputs_embeds[special_video_mask].numel() != video_features.numel():
            raise ValueError(
        ...
        **kwargs: Unpack[TransformersKwargs],
        ...
        if pixel_values is not None:
            ...
            image_mask, _ = self.get_placeholder_mask(
                input_ids, inputs_embeds=inputs_embeds, image_features=image_embeds, num_images=len(geometry_encoder_inputs[0])
            )

            mask = input_ids == self.config.image_token_id
            geo_embed_mask = image_mask.clone()
            vision_mask = mask | (input_ids == self.config.vision_start_token_id) | (input_ids == self.config.vision_end_token_id)
            text_embeds = inputs_embeds[vision_mask == False].view(1,-1,self.config.text_config.hidden_size)
        ...
        if pixel_values_videos is not None:
            ...
            _, video_mask = self.get_placeholder_mask(
                input_ids, inputs_embeds=inputs_embeds, video_features=video_embeds, num_images=len(geometry_encoder_inputs[0])
            )
        ...
        if position_ids is None:
            ...
            if (prefill_compiled_stage or prefill_noncompiled_stage) or self.rope_deltas is None:
                position_ids, rope_deltas = self.get_rope_index(
                    ...
                    num_images=len(geometry_encoder_inputs[0]),
                )
    ...
    def print_trainable_parameters(self) -> None:
        return self.language_model.print_trainable_parameters()
