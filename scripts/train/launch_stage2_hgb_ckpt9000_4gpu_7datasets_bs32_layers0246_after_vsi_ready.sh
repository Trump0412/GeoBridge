...
# Full 7-dataset Stage2-HGB launcher for the 4-GPU / global64 / layers0246 branch.
# The filename keeps the historical "bs32" label for compatibility with prior notes.
# Run only after scripts/data/validate_vsi590k_paths.py reports missing_image_refs=0.
...
export TOTAL_BATCH_SIZE=${TOTAL_BATCH_SIZE:-$((NPROC_PER_NODE * PER_DEVICE_BATCH_SIZE * 2))}
...
VARIANT_NAME=${VARIANT_NAME:-stage2_hgb_ckpt9000_4gpu_7datasets_${BATCH_TAG}_layers0246_20260601}
...
export TMP_ROOT=${TMP_ROOT:-"/tmp/geobridge_stage2_tmp_4gpu_7d_${BATCH_TAG}_0246"}
...
if [ -n "${VSI_590K_ANNOTATION_PATH:-}" ]; then
  echo "[INFO] vsi_590k_annotation_override=${VSI_590K_ANNOTATION_PATH}"
fi
